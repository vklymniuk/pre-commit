...
//
var configFilePath = path.join(basePath, '.pre-commit.json');
...
if (exists(configFilePath)) {
  ...
  try {
    let configFileOptions = JSON.parse(fs.readFileSync(configFilePath));
    config = Object.assign(config, configFileOptions);
  } catch (e) {
    console.log('pre-commit:');
    console.log('pre-commit: Cannot parse configuration file', configFilePath)
    console.log('pre-commit:');
  }
}
...
// to work correctly.
//
var git = path.resolve(config['root'], '.git')
...
var hookRelativeUnixPath = hook.replace(path.resolve(config['root']), '.');
